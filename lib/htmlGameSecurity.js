...
function saveHtmlGameToVault(gameId, options) {
  ...
  if (options.rawCode && typeof options.rawCode === 'string' && options.rawCode.trim()) {
    ...
    if (ext === '.zip') {
      ...
      if (fs.existsSync(bundleDir)) {
        try { fs.rmSync(bundleDir, { recursive: true, force: true }); } catch (e) {}
      }
  ...
  try {
    ...
    if (err.code === 'EROFS' || process.env.VERCEL) {
      const tmpVaultDir = path.join('/tmp', 'vault', 'html-games', gameId);
      fs.mkdirSync(tmpVaultDir, { recursive: true });
      fs.writeFileSync(path.join(tmpVaultDir, 'game.vault'), encryptedPayload, 'utf8');
      fs.writeFileSync(path.join(tmpVaultDir, 'meta.json'), JSON.stringify(meta, null, 2), 'utf8');
      return meta;
    }
    throw err;
  }
...
function hasHtmlGameInVault(gameId) {
  ...
  if (fs.existsSync(vaultFile)) return true;
  const tmpFile = path.join('/tmp', 'vault', 'html-games', gameId, 'game.vault');
  return fs.existsSync(tmpFile);
}

function getSessionPayload(gameId) {
  let vaultFile = path.join(VAULT_DIR, gameId, 'game.vault');
  if (!fs.existsSync(vaultFile)) {
    vaultFile = path.join('/tmp', 'vault', 'html-games', gameId, 'game.vault');
  }
  if (!fs.existsSync(vaultFile)) {
    return null;
  }
...
function deleteHtmlGameFromVault(gameId) {
  try {
    ...
    console.warn('[html-game] Notice: Could not remove directory on read-only disk:', err.message);
  }
  try {
    const tmpDir = path.join('/tmp', 'vault', 'html-games', gameId);
