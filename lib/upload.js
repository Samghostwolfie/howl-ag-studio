...
const uploadCover = multer({ storage: coverStorage, fileFilter: imageFilter, limits: imageLimits });
const uploadScreenshots = multer({ storage: screenshotStorage, fileFilter: imageFilter, limits: imageLimits });
const uploadTeamPhoto = multer({ storage: teamStorage, fileFilter: imageFilter, limits: imageLimits });
const uploadBuild = multer({ storage: buildStorage, limits: { fileSize: BUILD_LIMIT_MB * 1024 * 1024 } });
...
/**
 * Resolves an uploaded file's URL, handling both local and Firebase Storage paths.
 * @param {string} subdir  the uploads subdirectory (covers, screenshots, team, builds)
...
function resolveUploadUrl(subdir, filename) {
  ...
  if (USE_FIREBASE_STORAGE) {
    try {
      const bucket = getBucket();
      return `https://storage.googleapis.com/${bucket.name}/uploads/${subdir}/${filename}`;
    } catch (e) {
      // Fallback to local path if bucket isn't available
