...
function makeStorage(subdir) {
  ...
  let dest = path.join(__dirname, '..', 'public', 'uploads', subdir);
  try {
    fs.mkdirSync(dest, { recursive: true });
  } catch (err) {
    try {
      dest = path.join('/tmp', 'uploads', subdir);
      fs.mkdirSync(dest, { recursive: true });
    } catch (e2) {
...
const uploadHtmlGame = multer ? multer({ storage: multer.memoryStorage(), limits: { fileSize: 50 * 1024 * 1024 } }) : dummyMulter;
