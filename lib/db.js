...
function scheduleFlush(name) {
  ...
  if (process.env.VERCEL) {
    pendingFlushPromise = flush().catch((err) => console.error('[db] Serverless flush error:', err.message));
    return;
  }
...
async function waitForPendingFlush() {
  if (dirty.size > 0 || flushing) {
    await flush();
  }
...
function write(name, data) {
  ...
  try {
    ...
    if (err.code === 'EROFS' || err.code === 'EACCES' || process.env.VERCEL) {
      console.warn('[db] Read-only filesystem (' + (err.code || 'VERCEL') + '). Cached in memory.');
      return;
    }
...
module.exports = { read, write, init, flush, close, importFromFiles, waitForPendingFlush, USE_PG, USE_FIREBASE, MODE };
